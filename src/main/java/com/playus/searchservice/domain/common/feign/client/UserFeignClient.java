...
import com.playus.searchservice.global.config.feign.FeignConfiguration;
...
@FeignClient(name = "userFeignClient", url = "${feign.user.url}", path = "/user/api",
             fallback = UserFeignFallback.class, configuration = FeignConfiguration.class)
